...
var Util = (function() {
  ...
  var API = {
    ...
    request: function(url, properties) {
      ...
      var maybeFetchRemaining = function(json, properties) {
        ...
        return Promise
          .all(pages.map(function(page) {
            return API.request(page, properties);
          }))
          .then(function(pages) {
      ...
      var cacheKey = API.getHashString(url + properties.join('-'));
      var storage = Util.storage(cacheKey, true);

      // A valid stored JSON response exists
      if (storage.isValid(HOUR)) {
        return new Promise(function(resolve) {
          resolve(maybeFetchRemaining(storage.get(), properties));
        });
      }
...
(function(Util) {
  ...
  // The number of plugin instances that have been created
  var numberPlugins = 0;
  ...
  var Plugin = function(el, options) {
    ...
    this.el = el;
    this.instanceNumber = numberPlugins++;
  ...
  /**
   * The option types for the plugin.
   *
   * @type {{}}
   */
  _proto.optionTypes = {};
  ...
  /**
   * The supported events for the plugin.
   *
   * @type {{}}
   */
  _proto.events = {};
  ...
  /**
   * Returns the options for the plugin instance.
   *
   * @param options
   * @returns {*}
   * @private
   ...
   * @returns {plugin}
   */
  Util.createPlugin = function(prototype) {
    var plugin = function(el, options) {
      Plugin.call(this, el, options);
    };

    // Extend the plugin prototype
    plugin.prototype = Object.create(Plugin.prototype);
    for (var key in prototype) {
      if (prototype.hasOwnProperty(key)) {
        plugin.prototype[key] = prototype[key];
      }
    }
